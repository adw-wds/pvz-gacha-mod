...
> 格式：**现象 → 证据 → 根因链 → 处置 → 防复发 → 验证**。
> 遇到相似现象时**先按这里抓同一份证据**，不要凭感觉试。

---

## 案例 1 · 编译通过、装机后一用某功能就崩（`MissingMethodException`）

**现象**：`dotnet build` 0 错误，装到游戏里后调用某功能时游戏抛异常（日志/行为异常）。

**根因链**：本机**没有 .NET Framework 目标包** → MSBuild 用别处的 `mscorlib` 编译
→ 引用了游戏那份 `mscorlib` 里**不存在**的成员 → 编译期不报错，运行期才炸。

**处置**
```powershell
dotnet run --project 源码\API检查\API检查.csproj -c Release -- Verify 构建产物\PvzGachaMod.dll
# 它会拿每个成员引用去游戏程序集里核对；不通过会点名具体成员
```
把点名的成员换成等价写法，或确认引用路径（`FrameworkPathOverride` 必须指向游戏 `Managed`）。

**防复发**：把 `API检查 Verify` 写进构建流程（**每次构建后必跑**）。

---

## 案例 2 · "很多功能用不了"，但构建一路显示成功（静默装了两天旧 DLL）

**现象**：用户报多个功能都没反应；开发者这边每次构建都"成功"。

**证据**：`MOD\PvzGachaMod.dll` 的 `LastWriteTime` / 版本号（状态文件 `modVersion`）不是最新的。

**根因链**：`构建.ps1` 把产物写到了 `源码\MOD`，而安装器读的是交付根 `修改器\MOD`
→ 游戏里跑的一直是旧版，但**没有任何报错**。

**处置**：产物落到**最终生效位置**；复制后**立刻 SHA256 比对，不一致就 throw**。
并加**构建号**：构建时生成 `BuildStamp.g.cs`（`yyMMdd-HHmm`）→ 写进状态文件 `modVersion` → 面板显示"当前装的哪版"。

**防复发**：验收只看"游戏内状态文件的构建号"+"DLL SHA256"，**不看 build 输出**。
...
**证据**：反编译核对注入点实参：
```powershell
dotnet ilspycmd -t shangdian <Assembly-CSharp.dll> | Select-String "Hooks\."
```

**根因链**：`shangdian.buhuo` 的注入把第 1 个参数传成了 `data`，但正确实参是 `shop_List`
→ 游戏拿到错误对象 → 崩。

**处置**：按反编译结果修正实参；**`patch` 必须先从 `.orig` 重置再全量注入**（否则旧的错误注入会残留）。

**防复发**：任何 IL 改动后，**必须**反编译核对实参（列入流程）。

---

## 案例 4 · 面板永远显示"待生效"（失败通道没打通）

**现象**：用户改了设置，面板一直显示「待生效」，功能也没生效；用户结论"这功能是坏的"。

**根因链**（两层）
1. `共享\CommandApi.cs` 的 `Error()` 原来是 **static**，只拼 `{"ok":false,...}`，
   **从不写** `LastActionOk` / `LastActionResult`
   → 任何失败（未知特性、参数越界、脚本语法错）都让状态文件继续显示**上一次的成功文案**
   → 面板在结构上就是瞎的：它只知道"值没变成我想要的"，不知道"命令被拒了"。
2. `ApplySnapshot` 对不认识的 key **静默 `continue`** → 旧面板连新 mod / 新面板连旧 mod 时那几项被悄悄丢弃。

**处置**
1. `Error` 改**实例方法**并写 `LastActionOk = false; LastActionResult = message;`
2. `ApplySnapshot` 收集 `unknown`，`applied>0` 时仍返回 `ok:true`（让已应用的项落盘），
   但 `LastActionOk=false` 并**点名**是哪些 key + 提示重跑安装；
3. 面板侧：`CommandErrorText()` 把原因显示在状态行 + 标题栏染警告色；「待生效」全部改叫「**同步中**」；
   **自愈重发**（每 2.5s 重发整份快照，幂等，**封顶 5 次**）→ 超限明说"已重试 5 次仍未被确认，多数是该项需要进关卡/进商店才有作用对象"。

**防复发（写进测试）**：`测试\ChannelTests.cs` 加回归断言"**命令失败必须写进状态**"。
**实测对照**：发 `{"no.such.key":1}` → 修前 `lastActionOk=true, lastResult="已同步 1 项设置"`；
修后 `lastActionOk=false, lastResult="游戏不认识这些设置项：no.such.key…"`。

**教训**：写「乐观 UI」时**必须同时把失败通道打通**。只上报成功结果的协议，
会让 UI 无法区分"还没到"和"被拒了"，最后全被用户读成"坏了"。

---

## 案例 5 · 点一下"应用到货架"，6 格全变空格（"刷新空气"）

见 **1.3.4**。根因：哨兵值复用了游戏的 9999（=空格）。
处置：独立哨兵 `-1` + 写存档前消解 + 面板把 9999 明确标注为"空格"。

---

## 案例 6 · "卡包全解锁"把四种卡包全踢出了货架

见 **1.6**。根因：那五个标记是**排除条件**（true = 已买过 → 别再刷），设计文档写反了。
处置：按反编译改正为置 **false**。

---

## 案例 7 · "解锁全部卡牌还是没用"（三层叠加）

见 **6.7 / 1.5**。三层：① 功能自己调用了被注入的方法 → 全集塌缩 ② 全集靠池子拼 → 漏卡（实测漏 47 号）③ 图鉴不重读。
处置按 1.5 的表执行。**验证**：实测 153 → **154**（补上漏掉的 47）。

---

## 案例 8 · 面板显示"未连接"，其实游戏好好的（失焦冻结被误判）

**现象**：用户切到面板（游戏失焦）→ 面板显示未连接 → 用户以为 mod 坏了。

**根因**：本作没开 Run In Background → 失焦时主循环停住 → 状态文件不再更新 → 面板按"状态文件新鲜度"判断 → 判成未连接。

**处置**：三分法（见 6.5 / 配方 12）：进程在 + 状态文件停住 = **已连接・游戏在后台暂停**，
并说明"改动切回游戏后生效" + 提供「切到游戏」按钮。

**防复发**：**绝不**把 `GameFrozen` 显示成"未连接"；UI 文案写清楚这是**常态**。

---

## 案例 9 · 勾选之后又自己弹回去

**根因**：面板没有乐观更新 —— 游戏失焦暂停期间，界面按**旧状态**重绘 → 用户看到的"弹回"。

**处置**：点击后先把值记进 `_pending[key]`，游戏回信一致才摘掉标记。

---

## 案例 10 · 每秒都点不中控件

**根因**：每次 Tick 整页重建 UI → 点击落在被替换掉的控件上。

**处置**：只在「有效值签名」变化时重绘（见 12.1）。

---

## 案例 11 · "卡包全解锁完全不能用"（三层里只有两层）

**根因**：`FeatureCatalog`（看得见）+ `ModSettings`（存得下）都齐，但**第 3 层消费点是空的**
（实现早就有，但只挂在一次性动作上，没接到开关）。

**审计手法**：取 `ModSettings.cs` 全部 public 字段 → 对每个字段在排除 `ModSettings.cs`/`FeatureCatalog.cs`/`*Tests.cs` 后 grep 引用点 →
**计数 0 = 空壳**（那次审计：33 个功能里恰好 2 个空壳：`UnlockPacks`、`FreeRefresh`）。
⚠️ **注意假实现**：消费点计数=2 也可能只是"判断一下 + 加计数器" → **必须打开方法体确认它在写游戏字段**。
...
**处置**：`FeatureCatalog.SaveJson/LoadJson`（扁平 key→value，走 `TrySet` 自动夹取+校验）；
**启动读、每条命令成功后写**。

---

## 案例 13 · 加了第 5 条注入规则，装完 `verify` 只报"已注入 4"（且无报错）

见 **1.15**。根因：安装器是自包含单文件，**规则编译期嵌入**。
处置：重跑 `发布.ps1`。判定：源码直跑报 5/5 而安装器报 4/5 = 安装器是旧的。

---

## 案例 14 · 规则改了，旧注入残留导致崩

**根因**：注入器的幂等检测**只看钩子方法名** → 规则改了以后"旧的（错误的）注入"会被当成"已注入"跳过并残留。

**处置**：`patch` **每次都先 `File.Copy(backup, dll, overwrite:true)` 从原始备份重置**，再做全量注入。

---

## 案例 15 · BepInEx 装了但游戏没反应（无 `LogOutput.log`）

**现象**：游戏能跑，但 `BepInEx\LogOutput.log` 不存在，只有 `preloader_<时间戳>.log` 报 `MissingMethodException`。

**根因**：游戏 `mscorlib.dll` 被 Unity 链接器裁剪：
`System.Reflection.Module.GetPEKind` ✗ → `PlatformUtils.SetPlatform()` 崩；
绕过它后 `System.Reflection.Assembly.LoadFile` ✗ → `PreloaderRunner.LoadCriticalAssemblies()` 又崩。
（**Doorstop 本身是好的**：`winhttp.dll` 代理被加载并执行到了我们的代码。）

**处置**：**放弃 BepInEx**，改用本工程的"编译期 Cecil 注入 + 全局启动点"（1.13）方案。
**防复发**：写进文档（本工程 1.4 就是这条）。

---

## 案例 16 · 想用 TCP 通信，编译直接报 `CS1069`

**根因**：游戏程序集被裁剪，**没有 `TcpListener`**。

**处置**：改用**文件命令通道**（1.10）。**不要**再尝试别的网络 API（`Socket` 同类风险）。

---

## 案例 17 · 脚本写错了，面板却显示"已处理"

**根因**：失败没走失败通道（见案例 4）。

**处置**：脚本解析/执行失败必须 `LastActionOk=false` + 具体原因（哪个 token、第几个字符）。
**验证**：故意写错一个指令 → 面板必须显示具体语法错误。

---

## 案例 18 · 自动化调 `安装.cmd` 一直挂着不返回
...
## 案例 19 · 中文脚本报 `parse error`（引号配对错）

**根因**：PowerShell **5.1** 读**无 BOM 的 UTF-8 中文 `.ps1`** 会按 ANSI 解析 → 引号/括号配对错。

**处置**：一律用 `pwsh -File <脚本>`（PowerShell 7）；脚本内容尽量**纯 ASCII**；
需要匹配中文进程名时用 `Get-Process | Where-Object { $_.ProcessName -like '*PVZ*' }`（不要写中文字面量）。

---

## 案例 20 · 打包后多出一个同名无扩展名目录（几百 MB，删不掉）

**根因**：`Compress-Archive -Path <目录> -DestinationPath x.zip -Force` 覆盖已存在 zip 时的副作用。

**处置**：用 .NET `[System.IO.Compression.ZipFile]::CreateFromDirectory(...)`；
若残留目录被占用删不掉：先找到占用它的进程（本工程有 `工具脚本` 可辅助），停掉再 `cmd /c rmdir /s /q`。
...
| # | 坑 | 对策 |
|---|---|---|
| 1 | 编译过、运行 MissingMethodException | 每次构建后跑 `API检查 Verify` |
| 2 | 产物落错目录 → 静默装旧版 | 落到最终位置 + SHA256 自检 + 构建号 |
| 3 | 注入传错实参 → 商店一开就崩 | 反编译核对实参；`patch` 从 `.orig` 重置 |
| 4 | 失败通道没写 → 永远"待生效" | `Error` 写 `LastActionOk/Result`；面板显示原因 + 自愈重发封顶 |
| 5 | 哨兵复用游戏语义值 → "刷新空气" | 独立哨兵 -1；写存档前消解 |
| 6 | 排除条件语义写反 → 卡包被踢出货架 | 按反编译纠正；文档别当事实 |
| 7 | 功能调用了自己的注入点 → 结果悄悄变小 | 旁路标志（枚举期间放行 + 不计诊断） |
| 8 | 全集靠池子拼 → 漏卡 | 用权威卡牌表（三处取最大）+ 明说可能漏卡 |
| 9 | 图鉴不重读 | 解锁后 `LoadScene` 重载 |
| 10 | 失焦被显示成"未连接" | 三分法：已连接/已连接・后台暂停/未连接 |
| 11 | 勾选弹回 | 乐观更新（`_pending`） |
| 12 | 点不中控件 | 不整页重建；只在有效值签名变化时重绘 |
| 13 | 只有 1、2 层的空壳功能 | 三层审计（grep 字段引用点）+ 打开方法体确认 |
| 14 | 关游戏设置回默认 | 持久化 `修改器设置.json`（启动读、命令成功后写） |
| 15 | 改规则但安装器没变 | 重跑 `发布.ps1`；核对包内 `verify` 规则名 |
| 16 | 幂等残留旧注入 | `patch` 每次从 `.orig` 全量重置 |
| 17 | 用 BepInEx | 本作不可用（缺 GetPEKind/LoadFile） |
| 18 | 用 TCP/HTTP 通信 | 缺 `TcpListener` → 文件通道 |
| 19 | 用 LINQ / AppendAllText / DateTime.ToString(格式) | 被裁 → 手写循环 / 读+写 / 帧计数 |
| 20 | 脚本错误静默通过 | 失败要写原因 |
| 21 | 自动化调带 `pause` 的 cmd | 直接调 exe |
| 22 | PS 5.1 读中文脚本报 parse error | `pwsh -File`；脚本纯 ASCII |
| 23 | `Compress-Archive -Force` 产生同名目录 | 用 .NET `ZipFile` |
| 24 | 打包后说明文档过期 | 打包前 `Select-String` 扫过期字样/数字 |
| 25 | 改 md5 后显示"作弊者" | `SaveData.Save` 统一重算 3 次 MD5 |
| 26 | 缓存存档对象 | 每次重新 `Load`（游戏不缓存，你也别缓存） |
| 27 | 加速功能在暂停界面乱来 | 判 `Time.timeScale <= 0` 不覆盖 |
| 28 | 拿 `First.jianbukecui` 当零冷却开关 | 它参与出怪与阳光逻辑 → 改 `CardClick.Update` + `pro.nowcoolDown` |
| 29 | 改完货架要重进商店 | 主动调 `shuaxinHuojia()` |
| 30 | 垫钱法撞 int 上限 | 接近上限就不垫 |
| 31 | IL 参数编号搞错（实例 arg0=this） | 规范里写 IL 编号；代码里 `IsStatic ? i : i-1` |
| 32 | Cecil `MakeArrayType()` 不存在 | 用 `new ArrayType(t)` / `new ByReferenceType(t)` |
| 33 | 启动点重复注册/不幂等 | 用 `注册启动点.ps1`（带 `.orig`、幂等、可 -Unregister） |
| 34 | 卸载后残留加载错误 | 卸载时恢复 `ScriptingAssemblies.json` / `RuntimeInitializeOnLoads.json` |
| 35 | 面板发布失败（exe 被占用） | 打包前关面板进程 |
| 36 | 只改面板却重装了 mod | 只改面板时无需重装 mod（走 `dotnet publish`） |
| 37 | 旧面板连新 mod / 新面板连旧 mod | `unknown` key 要上报，不能静默丢 |
| 38 | 一次改多处再统一验证 | 一次一个变量 |
| 39 | 只看"编译 0 错误"就宣布完成 | 编译 + API检查 + 安装 + SHA256 四步自证 |
| 40 | 文档里写死"N 项/N 个分组" | 一定会过期；打包前扫一遍 |
